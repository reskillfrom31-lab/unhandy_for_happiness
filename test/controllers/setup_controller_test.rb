...
class SetupControllerTest < ActionDispatch::IntegrationTest
  ...
  # テスト: 準備完了メッセージとアカウント作成への導線が表示されることを確認
  test "should display completion message and account creation link" do
    get setup_index_path
    assert_response :success
    assert_match /準備完了/, response.body
    assert_match /アカウントを作成する/, response.body
  end
  ...
  test "should display navbar with logout button when authenticated" do
    ...
    assert_no_match /<a[^>]*class="[^"]*navbar-button[^"]*"[^>]*>.*ログイン.*<\/a>/m, response.body
  end
end
